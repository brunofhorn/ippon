...
export const EmptyPage = ({ className, title, description, buttons = [] }: IEmptyPage) => {
    return (
        <div
            className={twMerge(
                className,
                "bg-gray_five rounded-3xl p-10 h-screen w-full flex flex-col justify-center items-center",
            )}
        >
            <img src="https://ippon-frontend-web.vercel.app/static/media/no-data.3b77aebc7e95b60272bf1328d7809b83.svg" />
            <Typography variant="h1" className="text-black_four text-center mt-6">
                {title}
            </Typography>
            <Typography variant="h2" className="mt-4 text-black_four text-center">
                {description}
            </Typography>
            {buttons?.length > 0 && (
